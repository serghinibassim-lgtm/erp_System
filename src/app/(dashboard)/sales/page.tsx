              ...
              {loading ? (
                ...
                sales.map((s) => (
                  <tr key={s.id} className="border-b border-border/60 transition-colors even:bg-muted/20 hover:bg-muted/40">
                    ...
                    <td className="p-3 align-middle whitespace-nowrap">{s.saleNumber || "-"}</td>
                    <td className="p-3 align-middle whitespace-nowrap">{s.product.code} - {s.product.designation}</td>
                    <td className="p-3 align-middle whitespace-nowrap">{s.client?.name || "-"}</td>
                    <td className="p-3 align-middle whitespace-nowrap">{s.quantity}</td>
                    <td className="p-3 align-middle whitespace-nowrap">{Number(s.unitPrice).toFixed(2)}</td>
                    <td className="p-3 align-middle whitespace-nowrap">{Number(s.totalAmount).toFixed(2)}</td>
                    <td className="p-3 align-middle whitespace-nowrap">
                      {s.alert ? <span className="inline-flex h-6 w-fit items-center rounded-full border border-transparent px-2.5 py-0.5 text-sm font-medium whitespace-nowrap bg-destructive/10 text-destructive">Alerte</span> : <span className="inline-flex h-6 w-fit items-center rounded-full border border-transparent px-2.5 py-0.5 text-sm font-medium whitespace-nowrap bg-secondary text-secondary-foreground">OK</span>}
                    </td>
                  </tr>
                ))
              )}
