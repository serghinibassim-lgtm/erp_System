...
export async function POST(request: Request) {
  try {
    ...
    const { date, documentNumber, supplierId, productId, quantity, unitPrice, totalAmount, paymentMethod, observation } = body;
    ...
    const deviation = price > Number(product.purchaseRefPrice)
      ? price - Number(product.purchaseRefPrice)
      : null;

    const thresholdParam = await prisma.parameter.findUnique({ where: { key: "purchase_alert_threshold" } });
    ...
    const purchase = await prisma.$transaction(async (tx) => {
      ...
      if (stock) {
        const newTotalPurchases = stock.totalPurchases + qty;
        const newCurrentStock = stock.initialStock + newTotalPurchases - stock.totalSales;
